...
func query(req *dns.Msg, dnsAddresses []string) []dns.RR {
	...
	answer := common.ReadCache(domain, qtype, int64(opt.Get().ConnectOptions.DnsCacheTtl))
	if answer != nil {
	...
	common.WriteCache(domain, qtype, []dns.RR{}, time.Now().Unix() - int64(opt.Get().ConnectOptions.DnsCacheTtl) / 2)
	return []dns.RR{}
}
