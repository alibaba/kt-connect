...
func Reset(args []string) error {
	if resetAll {
		return saveConfig(make(map[string]interface{}))
	}
	if len(args) < 1 {
...
func resetConfigValue(config map[string]interface{}, key string) error {
	group, item, err := parseConfigItem(key)
	if err != nil {
		return err
	}
	if _, exist := config[group]; exist {
		delete(config[group].(map[string]interface{}), item)
	}
	return nil
}
